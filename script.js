...
async function fetchPosts() {
    try {
        ...
        if (res.status === 403) {
            showNotification("GitHub API 请求频率限制，请稍后再试。", "warning");
            return;
        }
...
function openPost(num) {
    ...
    if (!issue) {
        showNotification("文章内容已失效，请刷新页面。", "error");
        return;
    }
    ...
    htmlContent = htmlContent.replace(/<blockquote>\s*<p>\[!(NOTE|TIP|IMPORTANT|WARNING|CAUTION|AI)\]([\s\S]*?)<\/p>\s*<\/blockquote>/gi, (match, type, content) => {
        ...
        const titleText = t === 'AI' ? 'AI Generated' : t;
        return `
...
function playMusic() { audio.play().catch(e => showNotification("播放失败，请检查网络", "warning")); musicBtn.classList.add('playing'); playerBar.classList.add('is-playing'); iconPath.setAttribute('d', ICON_PAUSE); }
...
document.getElementById('search-input').oninput = (e) => {
    ...
    const filtered = allIssues.filter(i => 
        ...
        i.labels.some(l => l.name.toLowerCase().includes(term))
    );
